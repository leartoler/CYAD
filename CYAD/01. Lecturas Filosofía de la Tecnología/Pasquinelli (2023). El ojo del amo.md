---
Título:
Autor:
Año:
Editorial:
Libro:
Leído:
APA:
Audio:
Consultado:
---


# <mark style="background: #FFB8EBA6;">Concepto</mark>

(Concepto::cibernética)
	Acuñado por [[Norbert Wiener]] como la capacidad de un sistema para controlarse a si mismo mediante el intercambio de información con el entorno.

(Concepto::Máquinas de estado discreto (144))
	Máquina que pasan de mediante saltos o clics repentintinos a un estado bastante definido a 

(Concepto::red)
	Cualquier conjunto de esntidades físicas intrconectadas y mesurablemente activas

---
# <mark style="background: #FFF3A3A6;">Tema</mark>

## 6. La autoorganización de la mente cibernética

### 1. Una historia social del sistema nervioso

(Tema::Biología e información. Autoorganización (138))

(Tema::Lógica del umbral (140))

(Tema::Cerebro y máquina (140))
###### Escrito

![[Pasted image 20260702191411.png]]

![[Pasted image 20260702191514.png]]

![[Pasted image 20260702191543.png]]


### 2. Mecanizar la autoorganización

(Tema::Autoorganización y política (142))

(Tema::Máquina y mente (142-143))

(Tema::Información (143))

(Tema::Autoorganización, conexión y neuronas (146))

(Tema::Teoría hebbiana de asamblea celular (146))
###### Escrito

![[Pasted image 20260702191725.png]]

![[Pasted image 20260702191748.png]]

![[Pasted image 20260702195706.png]]


### 2a. Las teorías de la autoorganización y las primeras computadoras digitales

(Tema::El constructor universal (157))

###### Escrito

![[Pasted image 20260702192033.png]]


### 3. Preocupaciones militares por las redes autoorganizadas

(Tema::Temas de las redes neuronales (152))

(Temas::Autoorganización y política (152))
[[Friederich Hayek]]

###### Escrito

![[Pasted image 20260702192149.png]]


![[Pasted image 20260702192236.png]]


### 4. De la información lineal a la autoorganizada

(Tema::Perceptron y autoorganización (153))
	#perceptron

(Tema::Escuela simbólica es conexionista (155))

###### Escrito

![[Pasted image 20260702192414.png]]


### 5. Pensamiento computacionales y analogías mecanicistas de la mente

(Tema::Problemas de la cibernética (156))

(Tema::Lo que revela la cibernética (157))

(Tema::Problemas de la cibernética (157))

(Tema::Teoría laboral de la automatización (158))

(Tema::Hipótesis externalista (158))

(Tema::Pioneros de la IA (158))

###### Escrito

![[Pasted image 20260702192546.png]]

![[Pasted image 20260702192626.png]]

![[Pasted image 20260702192653.png]]


### 6. Autonomía y autoorganización

(Tema::Época en la que emergió la autonomía y autoorganización (159))

(Tema::Dos caras del proyecto de autonomía (159))

(Tema::Pronunciación en contra de la automatización realizado por la contracultura que aún sigue vigente (160))

(Tema::Autonomía según la cibernética (161))

(Tema::IA y sociedad (163))
###### Escrito

![[Pasted image 20260702192911.png]]

![[Pasted image 20260702194857.png]]


## 7. La automatización del reconocimiento de patrones

### 1. La controversia sobre la percepción Gestalt

(Tema::Cerebro vs. escuela del Gestalt (165))

(Tema::Escuela del Gestalt (166))

###### Escrito

![[Percepcion_Gestalt.png]]


![[Von_Neumann.png]]


### 2. La invención de las redes neuronales artificiales

(Tema::Inducción vs. deducción (169))

(Tema::Redes neuronales artificiales (170))
	#máquinas_inductivas

(Tema::Cambios importantes con las redes neuronales (173))
	#McCulloch #Pitts

###### Escrito

![[invención_redes_neuronales_artificiales.png]]


### 3. Lo que el ojo de la rana le dice al cerebro de la rana

(Tema::Campos de fuerza::174-175)
	#Campos_de_fuerza #gestalt 

(Tema::El ojo y la cognición , del percepto al concepto (177))
	#Lettuin #Maturana #McCulloch #Pitts 

###### Escrito 

![[Campos_de_fuerza.png]]


### 4. El lenguaje del cerebro no es el lenguaje de las matemáticas

(Tema::Ideas de Von Neumann (180-181))
	#Von_Neumann 

(Tema::Sobre el error (181))

(Tema::Hayek y la modelización (184))
	#Hayek

###### Escrito

![[Lenguaje_Matematicas.png]]



---
# <mark style="background: #FFB86CA6;">Nota</mark> 

(Nota::La historia de los modelos cerebrales es una historia de las tecnologías literarias y materiales (137))

(Nota::Algoritmo Alexnet y el aprendizaje profundo (138))

---

(Nota::Más allá de la conferencia de Dartmouth en 1956, la de 1950 en las oficinas de investigación naval de EUA, a finales de 1950, dan indicio de la importancia y raices que estaban teniendo la investigación en la IA (150))
	#Conferencia_Darthmouth

(Nota::Las redes neuronales buscaban imitar la neuroplasticidad. Se buscaba un principio de una "red infalible de neuronas falibles". Así, las neuronas "inútiles" trabajan en paralelo y pueden dar una respuesta correcta (151))
	#Redes_neuronales

(Nota::Proyecto sybersyn (1971), como ejemplo de autoorganización. Creado en el gobierno de Allende, era centralizado y fue contemporáneo a ARPANET (152-153))
	#Cybersyn

---
(Nota::El perceptrón y las redes neuronales marcaron un giro topológico. Tomaba en consideración al espacio no la información de manera lineal (155))

---
(Nota::Seguir una norma siempre tendrá un significado diferente para un ser humano y para una máquina. Wittgenstein (162))

---
(Nota::Los conexionistas creían en que el reconocimiento de un patrón significaba razonamiento lógico (167))
	#conexionistas

(Nota::Von neumann propuso una síntesis entre el gestalt y la lógica computacional, mismo que inspiró la red neuronal perceptron de Roseblatt que explicaba el cálculo estadístico (167))
	#Von_Neumann #gestalt #perceptron 

(Nota::Sobre la topología en la visión artificial (168))

---

(Nota::Con Leibniz se buscó la inteligencia de las máquinas con el postulado de que el humano puede expresar su inteligencia por medio de una lógica proposicional, el cual es un tipo de pensamiento lineal como el lenguaje o la máquina de turing (169))
	#Leibniz #máquina_de_Turing

---

(Nota::Las redes neuronales tienen una arquitectura destribuida. Esta memoria distribuida due una gran inspiración para el perceptron (182))
	#perceptron 


---
# <mark style="background: #ADD8E6;">Idea</mark>

(Idea::Durante una época se adaptaban lo mecánico y lo tecnológico a lo cultural y físico. Posible empacho de una tecnofanía. La máquina como modelo ontológico de un todo)

(Idea::Los estudios de la cibernética dejaron también en un lado el percepto de "El todo es más que la suma de sus partes")


---
### <mark style="background: #FF5582A6;">Pregunta</mark> 

(Pregunta::¿Cuál es el modelo de los campos de fuerza?)

---
### <mark style="background: #BBFABBA6;">Esquema</mark>

---
### <mark style="background: #ADCCFFA6;">HT</mark>

---

# <mark style="background: #ADD7B9;">Investigar</mark>

- investigar a [[Ross Ashby]]
- Investigar a [[Donald Hebb]], neuropsicólogo que identificó las reglas básicas de la autootrganización en las redes neuronales
- Investigar a [[Zuze]] y su teoría del cálculo del espacio y los autómatas celulares para explicar el universo y todo lo demás.
- [[Turing]] y su ensayo sobre la base química de la morfogénesis y de cómo los organismos actúan como actores que se computan a si mismos y expresan estructuras autopoiéticas

---
