---
Título:
Autor:
Año:
Editorial:
Libro:
Leído:
APA:
Audio:
Consultado:
---
### <mark style="background: #FFB8EBA6;">Concepto</mark>

(Concepto::Samandari)
	Quien hace nacer el conflicto

(Concepto::Entropología)
	Un saber que mantiene de forma permanente una obsesión por la entropía, la pérdida y el desorden

---
# <mark style="background: #FFF3A3A6;">Tema</mark>

## 1. El mito proclama el orden primordial

(Tema::Mito y ciencia (17))

(Tema::Sobre el mito y sus tres características (18

##### Escrito

![[Pasted image 20260605235251.png]]


##### En el comienzo era caos

(Tema::Función del juego y lo arbitrario (23))

(Tema::El mito de los nuevos comienzos (25))

##### Escrito

![[Pasted image 20260606002002.png]]

![[Pasted image 20260606002020.png]]


##### El rito trabaja para el orden

(Tema::Relación entre el rito y el juego (29))

(Tema::Rito e instrumento (29))

##### Escrito

![[Pasted image 20260606002313.png]]


##### La tradición obra con astucia frente al movimiento

(Tema::Tradición (35))

(Tema::Las sociedades (35))

##### Escrito

![[Pasted image 20260606011055.png]]

## 5. El orden se traduce en orden

(Tema::Orden y desorden (112))

(Tema::El desorden y el juego (112-113))

(Tema::Correctores del desorden (114))

##### Escrito

![[Pasted image 20260605192255.png]]

![[Pasted image 20260606011454.png]]


##### El mundo al revés

(Tema::La inversión en lo social (115))

(Tema::La fiesta (118))

(Tema::La fiesta del asno (119))

(Tema::Invsersión y efervescencia colectiva (119))

(Tema::Sobre el carnaval (119))

(Tema::La cencerrada (122))

##### Escrito

![[Pasted image 20260606011955.png]]


##### Los que sobrepasan los límites

(Tema::Sobre los Dioniosios (127))

(Tema::Naturaleza del trickster (131))

##### Escrito

![[Pasted image 20260606013018.png]]


##### Poder por el desorden

(Tema::Espacio y desorden (132))

(Tema::El trabajo del clown (134-135))

(Tema::Rituales como continuación (137))

##### Escrito

![[Pasted image 20260706101744.png]]


---
### <mark style="background: #FFB86CA6;">Nota</mark> 

(Nota::Inversión y conversión del desorden y de quienes lo producen: los contrarios)

(Nota::El desorden como principio fundador)

(Nota::El mito y la creación del mundo a partir del desorden)

(Nota::El rito es orden. Está estructurado y constituye un sistema de comunicación y creación de gran complejidad. Está adaptado a un fin)

(Nota::En la fiesta no hay subversión, es más una válvula de escape que otra cosa, es una pausa que hace descansar (119))

(Nota::La parodía, el orden originado a partir del desorden, el orden simulado, se convierte en una crítica indirecta (120))

(Nota::Importancia del cuerpo y la sexualidad en el carnaval)

(Nota::El carnaval como válvula de escape, como algo psicológico (121-122))

---
### <mark style="background: #ADD8E6;">Idea</mark>

(Idea::Hay alguna relación entre sexualidad y el orden y desorden, entre la relación de ambos)



---
### <mark style="background: #FF5582A6;">Pregunta</mark> 

---
### <mark style="background: #BBFABBA6;">Esquema</mark>

---
### <mark style="background: #ADCCFFA6;">HT</mark>

---
